...
namespace Minixer.Infrastructure.RemoteFramebuffer.Encodings
{
	...
	public sealed class ZrleRectangle : EncodedRectangle
	{
		...
		private readonly Pixel[] palette = new Pixel[128];
		private readonly Pixel[] tileBuffer = new Pixel[TILE_WIDTH * TILE_HEIGHT];
		...
		private void ReadZrlePackedPixels(int tw, int th, Pixel[] palette, int palSize, Pixel[] tile)
		{
		...
		private void ReadZrlePlainRLEPixels(int tw, int th, Pixel[] tileBuffer)
		{
			var ptr = 0;
			var end = ptr + tw * th;
			while (ptr < end) {
		...
		private void ReadZrlePackedRLEPixels(int tx, int ty, int tw, int th, Pixel[] palette, Pixel[] tile)
		{
			var ptr = 0;
			var end = ptr + tw * th;
			while (ptr < end) {
